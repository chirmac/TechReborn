...
import techreborn.api.recipe.recipes.FissionReactorRecipe;
...
public class FissionControlComputerBlockEntity extends GenericMachineBlockEntity implements BuiltScreenHandlerProvider {

	public RebornInventory<FissionControlComputerBlockEntity> inventory;
	...
	FissionReactorRecipe currentRecipe = null;
	Identifier currentRecipeID = null;
	boolean hasStartedCrafting = false;
	boolean checkNBTRecipe = false;
	long lastTick = -1;

	public FissionControlComputerBlockEntity() {
		super(TRBlockEntities.FISSION_CONTROL_COMPUTER, "FissionControlComputer", -1, -1, TRContent.Machine.FISSION_CONTROL_COMPUTER.block, -1);
		this.inventory = new RebornInventory<>(3, "FissionControlComputerBlockEntity", 64, this);
	}
	...
	public int getProgressScaled(int scale) {
		FissionReactorRecipe reactorRecipe = getCurrentRecipeFromID();
	...
	private void updateCurrentRecipe() {
		for (RebornRecipe recipe : ModRecipes.FISSION_REACTOR.getRecipes(getWorld())) {
			if (validateRecipe((FissionReactorRecipe) recipe)) {
				currentRecipe = (FissionReactorRecipe) recipe;
				craftingTickTime = 0;
	 ...
	 */
	private boolean validateRecipe(FissionReactorRecipe recipe) {
	...
	public void tick() {
		...
		if (checkNBTRecipe) {
			...
			for (final RebornRecipe reactorRecipe : ModRecipes.FUSION_REACTOR.getRecipes(getWorld())) {
				if (validateRecipe((FissionReactorRecipe) reactorRecipe)) {
					this.currentRecipe = (FissionReactorRecipe) reactorRecipe;
				}
			}
		}
	...
	public void fromTag(BlockState blockState, final CompoundTag tagCompound) {
		...
		this.craftingTickTime = tagCompound.getInt("crafingTickTime");
	...
	public CompoundTag toTag(final CompoundTag tagCompound) {
		...
		tagCompound.putInt("crafingTickTime", this.craftingTickTime);
	...
	// ItemHandlerProvider
	@Override
	public RebornInventory<FissionControlComputerBlockEntity> getInventory() {
		return inventory;
	}
	...
	public FissionReactorRecipe getCurrentRecipeFromID() {
		if (currentRecipeID == null) return null;
		return ModRecipes.FISSION_REACTOR.getRecipes(world).stream()
	...
	public Text getStateText() {
		if (state == -1) {
			...
			FissionReactorRecipe r = getCurrentRecipeFromID();
			if (r == null) {
